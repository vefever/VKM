...
import { FolderOpen, CalendarClock, Activity, ImagePlus, FolderDown, ChevronDown, Loader2 } from "lucide-react";
...
export type WeekFileGroup = { weekNo: number; files: Attachment[] };
export type DayFileGroup = { day: number; habitFiles: { label: string; files: Attachment[] }[] };

type ZipEntry = { folder: string; file: Attachment };

// Normalized file-browsing UI: day/week grouped display + "download all as
// zip" — reused by the single-participant Files tab and the admin bulk Files
// page so the ~150 lines of grouping/zip logic exist in exactly one place.
    ...
    return (
      <SectionCard>
        <p className="flex items-center gap-2 py-6 text-center text-sm text-muted-foreground">
          <FolderOpen className="h-4 w-4" /> {emptyLabel}
        </p>
      </SectionCard>
    );
  }
