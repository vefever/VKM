...
import { useEmailLog, type EmailLogRow } from "@/components/admin/messaging-data";

// Human labels + accent for each email `kind` written by the messaging fn.
const KIND_META: Record<string, { label: string; className: string }> = {
  otp: { label: "Login code", className: "bg-[oklch(0.93_0.05_255)] text-[oklch(0.4_0.13_255)]" },
  mfa: { label: "2FA code", className: "bg-[oklch(0.93_0.05_290)] text-[oklch(0.42_0.14_290)]" },
  reminder: { label: "Reminder", className: "bg-gold/15 text-[oklch(0.45_0.1_85)]" },
  bulk: { label: "Bulk / invite", className: "bg-secondary text-foreground" },
...
const KIND_FILTERS = ["all", "otp", "mfa", "reminder", "bulk", "admin", "test"] as const;
...
// The email audit log — every email the platform has sent, newest first,
// live-updating. Reused on the Messaging "Email log" tab and the Workflow
// Automation page. `embedded` renders without the outer PageHeader spacing.
export function EmailLogPanel({ limit = 200 }: { limit?: number }) {
  const { rows, loading, reload } = useEmailLog(limit);
    ...
    <SectionCard
      title={
        <span className="flex items-center gap-2">
          <Mail className="h-4 w-4 text-navy" /> Email log
        </span>
      }
      subtitle="Every email the platform has sent — updates live"
      action={
    ...
    >
      ...
      {loading ? (
        ...
        <p className="py-8 text-center text-sm text-muted-foreground">
          {rows.length === 0 ? "No emails sent yet." : "No emails match this filter."}
        </p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] text-sm">
            ...
            <tbody>
              {filtered.map((r) => (
                <EmailLogRowView key={r.id} row={r} />
              ))}
            </tbody>
          </table>
        </div>
...
function EmailLogRowView({ row }: { row: EmailLogRow }) {
  ...
  return (
    <tr className="border-b border-border/60 last:border-0 align-top">
      ...
      <td className="py-2.5 pr-3 font-medium text-foreground">{row.to_email}</td>
      <td className="py-2.5 pr-3">
        <span className={cn("inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium", meta.className)}>
          {meta.label}
        </span>
      </td>
      <td className="max-w-[240px] truncate py-2.5 pr-3 text-foreground" title={row.subject ?? ""}>
        {row.subject ?? "—"}
      </td>
      <td className="py-2.5">
        {failed ? (
