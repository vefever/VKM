...
### Added
- **Per-member 1-on-1 session videos (by week)** — you can now attach private session videos to a **specific member**, organised across the 16-week program. A new **Admin → Member Sessions** page lets you search a member, pick a week (or "General"), and either **paste a link** (Google Drive, YouTube, Vimeo, or any direct video URL) **or upload a file** — with an optional title and note. Each member gets a new **"My Sessions"** page that shows their videos week by week (Week 1 → 16, plus General), and **only that member can see their own** (enforced at the database level). Google Drive videos now stream in-app (make sure the Drive file is shared "Anyone with the link · Viewer").
