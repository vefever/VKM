-- Add mrr_inr (each participant's business monthly recurring revenue) to
-- admin_live_participants so the System Overview can show real cohort revenue
-- (Σ mrr_inr) instead of the old mock "₹48L". Return-type change → drop first.
drop function if exists public.admin_live_participants(uuid);
